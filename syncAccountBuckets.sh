...
aws --region ${REGION} s3 ls s3://code.${DOMAIN}/ > /dev/null 2>&1
RESULT=$?
if [[ "$RESULT" -ne 0 ]]; then
...
aws --region ${REGION} s3 sync ${DRYRUN} ${DELETE} --exclude=".git*" bootstrap/ s3://code.${DOMAIN}/bootstrap/

# Confirm access to the credentials bucket
aws --region ${REGION} s3 ls s3://credentials.${DOMAIN}/ > /dev/null 2>&1
RESULT=$?
if [[ "$RESULT" -ne 0 ]]; then
...
aws --region ${REGION} s3 sync ${DRYRUN} ${DELETE} . s3://credentials.${DOMAIN}/${AID}/alm/docker/
RESULT=$?
